...
export const Navbar = () => {
  ...
  return (
  <nav className={cn("fixed  w-full z-40 transition-all duration-300", isScrolled ? "py-3 bg-background" : " py-5 bg-background")} >

    <div className="container flex items-center justify-between">
      <a className=" text-xl font-bold text-primary flex items-center" href="#hero">
        <span className="relative z-10 ">
          <span className="text-glow text-foreground">Tested</span> Portfolio
        </span>
      </a>
      ...
      <div className="hidden md:flex space-x-8">
        {navItems.map((item, key) => (
          <a key={key} href={item.href} className="text-foregroud/80 hover:text-primary transition-colors duration-100">
            {item.name}
          </a>
        ))}
      </div>
      ...
      >
        <div className="flex flex-col space-y-8 text-xl">
          {navItems.map((item, key) => (
            ...
            onClick={() => setIsMenuOpen(false)} // click and go to clicked page;
            >
              {item.name}
            </a>
          ))}
